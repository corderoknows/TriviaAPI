...
class TriviaTestCase(unittest.TestCase):
    ...
    def test_paginate_question(self):
    ...
    def test_404_request_beyond_valid_page(self):
        """ Test error if user tries to access to nonexistent page"""
        response = self.client().get('/questions?page=1000')
        dat = json.loads(response.data)
        
        self.assertEqual(response.status_code, 404)
        self.assertEual(data['success'], False)
        self.assertEqual(data['message'], 'resource not found')
        
    def test_get_categories(self):
        """ Test success of loading categories"""
    ...
    def test_delete_question(self):
        ...
        # check status code, success message & compare length before
        self.assertEqual(response.status_code, 200)
        self.assertEqual(data['success'], True)
        self.assertEqual(data['delete'], q_id)
        self.assrtTrue(len(questions_before) len(questions_after) == 1)
    ...
    def test_create_question(self)
        ...
        data = json.loads(response.data)
        questions_after = (Questions.query.all())
    ...
    def test_422_create_question(self):
        ...
        # get questions before post. Create questions, load response data
        # and get num questions after
        questions_before = len(Questions.query.all())
        ...
        self.assertTrue(len(questions_before) == len(quest_after))
        
    def test_search_question(self)
        """test success of searching questions"""
        # send post requst with search term, load response data
        # new_search = {'searchTerm': 'a'}
        response = self.client().post('/questions/search', json={'searchTerm': 'place'})
        data = json.loads(response.data)
        ...
        self.assertIsNotNone(data['total_question'])
        
    
    def test_404_search_questions(self):
        """test for no search results 404"""
        response = self.client().post('/question/search', json={'searchTerm': ''})
        data = json.loads(response.data)
        
        # check status code, false success message
        self.assertEqual(response.status_code, 404)
        self.assertEqual(data['success'], False)
        self.assertEqual(data['message'], "research not found")
    ...
    def test_get_category_questions(self):
        ...
        # check status code success message
        # number of questions and current category
        self.assertEqual(response.status_code, 200)
        self.assertEqual(data['success'], True)
    ...
    def test_404_get_category_questions(self)
        """test for 404 error with no questions for category"""
